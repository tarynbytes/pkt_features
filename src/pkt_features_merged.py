...
def equalize_output(websites: list, x: int, y: int, z: int):
    """Filters out any websites where first_x, first_neg_y, or first_pos_z are less than x,y,z respectively to ensure
    each website has equal number of features."""
...
def output_csv(outfile: str, websites: list, x: int, y: int, z: int):
    ...
    csv_data = [['Website Number', 'Average Packet Size (Abs Value)', 'Average Negative Packet Size',
                 'Average Positive Packet Size',
                 'Total Packet Sizes: 10th Percentile', 'Total Packet Sizes: 25th Percentile',
                 'Total Packet Sizes: 50th Percentile',
                 'Total Packet Sizes: 75th Percentile', 'Total Packet Sizes: 90th Percentile',
                 'Negative Packet Sizes: 10th Percentile', 'Negative Packet Sizes: 25th Percentile',
                 'Negative Packet Sizes: 50th Percentile',
                 'Negative Packet Sizes: 75th Percentile', 'Negative Packet Sizes: 90th Percentile',
                 'Positive Packet Sizes: 10th Percentile', 'Positive Packet Sizes: 25th Percentile',
                 ...
                 'Cumulative Sum (including zeros)', 'Cumulative Sum (excluding zeros)', 'Cumulative Sum Negatives',
    ...
    for website in tqdm(websites, desc="Creating CSV file..."):
        csv_data.append([website.number, website.avg_pkt_size, website.avg_neg_pkt_size, website.avg_pos_pkt_size,
                         ...
                         website.count_positive, website.count_negative, website.count_zeros, website.smallest_pkt_size,
...
def generate_samples(sample_size: int, zeros: bool, websites: list):
    ...
    return [sample for sample in samples if counts[
        sample.website_number] > 1]  # Filter out samples that don't have at least two of the same website number
